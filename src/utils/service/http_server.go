...
func (server *Server) BaseHandler(w http.ResponseWriter, r *http.Request) {
	...
	//uid, err = server.mvaliduser(r)
	if err == nil {
		body, err = server.RequestHandler(vars["module"], "Base_"+vars["method"], 0, r, result, nil)
	}
	end := time.Now().UnixNano()
	server.Respose(w, r, err, body, result, end-start)
}
...
func (server *Server) RequestHandler(moduleName string, methodName string, uid uint32, r *http.Request, result map[string]interface{}, bodyBytes []byte) (byte []byte, e error) {
	fmt.Println(fmt.Sprintf("http.Request.Boby: %v", r.Body))
	...
	url := r.URL.RawQuery
	if e != nil {
		return bodyBytes, NewError(ERR_USER_NOT_FOUND, "read user db error : "+e.Error())
	}
	power := ud.Power
	if ok {
		method := reflect.ValueOf(module).MethodByName(methodName)
		if method.IsValid() {
			values = method.Call([]reflect.Value{reflect.ValueOf(&HttpRequest{urlencodedbody, jsonbody, bodyBytes, r, url, uid, power}), reflect.ValueOf(result)})
		} else {
