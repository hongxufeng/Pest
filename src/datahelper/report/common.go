...
func AppendWhere(req *service.HttpRequest, param *Param, buf *bytes.Buffer) error {
	...
	for _, colconfig := range param.ColConfigDict {
		if colconfig.ISMyself {
			hasWhere = true
			buf.WriteString(" where ")
			buf.WriteString("(")
			buf.WriteString(colconfig.Tag)
			buf.WriteString("=")
			buf.WriteString(function.Uint32ToString(req.Uid))
			buf.WriteString(")")
			buf.WriteString(" and ")
		}
		var value string
...
func BuildQuerySQL(req *service.HttpRequest, param *Param, settings *model.Settings) (sql string, err error) {
	...
	buf.WriteString(",")
	buf.WriteString(function.IntToString(settings.Rows))
	sql = buf.String()
	return
}
func BuildQueryThisSQL(req *service.HttpRequest, param *Param, settings *model.CRUDSettings) (sql string, err error) {
	if param.TableConfig.HasPower && param.Power > param.TableConfig.Power {
		err = service.NewError(service.ERR_POWER_DENIED, "您的用户权限不足啊！")
		return
	}
	...
	buf.WriteString(param.TableConfig.Name)
	buf.WriteString(" where ")
	buf.WriteString(settings.QueryKey)
	buf.WriteString("=")
	buf.WriteString(settings.QueryValue)
	sql = buf.String()
	return
}
func GetSelectQuery(hasWhere bool, req *service.HttpRequest, param *Param, fields string) (query string, err error) {
	var buf bytes.Buffer
	buf.WriteString("select ")
