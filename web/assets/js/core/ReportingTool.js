...
(function($) {
    ...
    $.fn.rt = function(options) {
        ...
        globalVars.queryObj.table = settings.table;
        ...
        var load = function(elem) {
            ...
            if (settings.query) {
                ...
                if (!hr) {
                    queryStr += "&hr=true";
                    window.history.pushState(null, null, location.pathname + queryStr);
                    postData(elem);
                } else {
                    postData(elem);
                }
        ...
        var format = function() {
            ...
            if (!selector.find("div").length || settings.selector !== true) {
        ...
        var postData = function(elem) {
            var postOpts = {
                ...
                success: function(data) {
                    ...
                    _this.html("<ol class=\"rt-nav breadcrumb\"></ol>\
                    ...
                    if (jsonObject.selector && settings.selector === true) {
        ...
        var expandNode = function() {
            ...
            if (arrow.hasClass("down")) {
                ...
                for (var t in trees) {
                    $.ajax({
                        async: false,
                        url: serverURL + "GetTable" + "?" + trees[t],
                        method: "POST",
                        data: {
                            table: encodeURI(t),
                            configFile: settings.configFile,
        ...
        var toggleAll = function() {
            var allCheckbox = _this.find(".rt-td-checkbox");
